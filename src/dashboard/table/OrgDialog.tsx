...
export class OrgDialog extends React.Component<OrgDialogProps, OrgDialogState> {
    ...
    private _load() {
        this._orgService
            .getContacts(this.props.org.idOrg, this.props.idBand)
            .then((contacts) => {
                this.setState(
                    {
                        ...this.state,
        ...
        return (
            ...
            >
                {this.state.notes.map((n) => {
                    const isEditable: boolean = n.user
                        ? n.user.id === this._noteService.userId
                        : false;
                    const isDeletable: boolean = this.props.bandAdmins
                        ? this.props.bandAdmins.findIndex(
                              (u) => u.id === this._noteService.userId
                          ) !== -1
                        : false;
                    return (
    ...
    render(): JSX.Element {
        ...
        const dialogInner: JSX.Element = this.state.loaded ? (
            <Card>
                <CardHeader title="Organisme"></CardHeader>
                <CardContent className="orgdialogcontent">
                    ...
                    <div className="orgdialogflextop">
                        ...
                        <div className="orgsecondhalf">
                            ...
                            {porg.category}
                            <br />
                            {dateFormatAny(porg.creationStamp.toLocaleString())}
                            <br />
                        </div>
                    </div>
                    <br />
                    ...
                    <div className="orginneractions">
                        ...
                        <Button
                            className="orginnerbutton"
                            onClick={this.handleNoteDialogOpen.bind(this)}
                        >
                            Ajouter note
                            <Icon>edit</Icon>
                        </Button>
                ...
                <NoteDialog
                    ...
                    key={
                        this.state.currentEditedNote
                            ? this.state.currentEditedNote.id
                            : 0
                    }
                />
                <ContactDialog
                    ...
                    key={
                        this.state.currentEditedContact
                            ? `${this.state.currentEditedContact.id}contdial`
                            : "0contdial"
                    }
                />
                <CardActions id="bandactions"></CardActions>
            </Card>
        ) : (
            <div>Chargement...</div>
        );
