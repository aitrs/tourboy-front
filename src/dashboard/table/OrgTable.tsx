...
export class OrgTable extends React.Component<OrgTableProps, OrgTableState> {
    ...
    componentDidUpdate(previousProps: OrgTableProps) {
        if (this.props.idBand) {
            if (
                !previousProps.idBand ||
                previousProps.idBand !== this.props.idBand
            ) {
                this.setState({
                    ...this.state,
                    page: 0,
    ...
    handleCheckBox(checked: boolean, org: Org) {
        if (checked) {
            ...
            const infex = this.state.selected.findIndex(
                (o) => o.idActivity === org.idActivity
            );
            if (infex !== -1) {
    ...
    isItemSelected(id: number): boolean {
        return this.state.selected.find((o) => o.idActivity === id) ===
            undefined
            ? false
            : true;
    }
    ...
    onFilterChange(col: Column, value: string) {
        const index = this.state.filters.findIndex((f) => {
            const cname = col.colname ? col.colname : col.ident;
            return f.key === cname;
        });
    ...
    handleStatusAffectation(org: Org, status: OrgStatus, idUser: number) {
        this._orgService
            .tag(this.props.idBand as number, [org], idUser, status)
            .then((resp) => {
                if (resp.tagged) {
                    this._retrieveData();
                } else {
    ...
    render(): JSX.Element {
        if (this.props.idBand && this.state.totalItems !== undefined) {
                                ...
                                <TableRow>
                                    <TableCell padding="checkbox">
                                        <Checkbox
                                            ...
                                            onChange={this.onSelectAllClick.bind(
                                                this
                                            )}
                                            inputProps={{
                                                "aria-label":
                                                    "Selectionner toutes les lignes visibles",
                                            }}
                                        />
                                    </TableCell>
                                    {this.state.layout.map((col) => (
                                        ...
                                        >
                                            ...
                                            <br />
                                            <form
                                                onSubmit={this.onFilterSubmit.bind(
                                                    this
                                                )}
                                            >
                                                <TextField
                            ...
                            <TableBody>
                                {this.state.data.map((row) => {
                                    ...
                                    const isAdmin = this.props.bandAdmins
                                        ? this.props.bandAdmins.findIndex(
                                              (ba) =>
                                                  ba.id ===
                                                  this._orgService.userId
                                          ) !== -1
                                        : undefined;
                                    const isAffected = isAdmin
                                        ? Boolean(row.userId)
                                        : row.userId
                                        ? row.userId === this._orgService.userId
                                        : undefined;
                                        ...
                                        >
                                            ...
                                            <TableCell align="right">
                                                <MenuActions
                                                    ...
                                                    onAffectation={this.handleAffectation.bind(
                                                        this
                                                    )}
                                                    onStatusAffectation={this.handleStatusAffectation.bind(
                                                        this
                                                    )}
                                                />
            ...
            return <p>Veuillez sélectionner un groupe</p>;
        }
    }
}
